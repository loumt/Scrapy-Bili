<template>
  <el-container>
    ...
    <el-main>
      <el-table :data="this.$store.state.AttentionCartoon.rows" class="shadow" max-height="100%" :stripe=true
                :border=true>
        ...
        <el-table-column align="center" label="操作" width="200" fixed="right">
          <template slot-scope="scope">
            <el-button type="danger" icon="el-icon-star-off" size="mini" title="取消关注" circle
                       @click="cancelAttention(scope.row)"></el-button>
          </template>
        </el-table-column>
      </el-table>
...
<script>
    ...
    export default {
        ...
        watch: {
            "cId": function () {
                this.$store.commit('AttentionCartoon/initPageOptions');
                this.initData();
            },
            "cName": function () {
                this.$store.commit('AttentionCartoon/initPageOptions');
